import { Mode, buildToolContracts, toolInputSchemas } from "shared";
...
function describeTools(): string {
  return Object.entries(buildToolContracts)
    .map(([name, contract]) => {
...
Available tools: ${Object.keys(buildToolContracts).join(", ")}`;
